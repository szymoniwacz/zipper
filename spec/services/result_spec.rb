  ...
  describe "#map" do
    ...
    context "when there is already an error" do
      it "returns self without calling the block" do
        error_result = Result.new(error: "Previous error")
  ...
  describe "#rescue" do
    ...
    context "when there is already an error" do
      it "returns a new Result with rescued error" do
        error_result = Result.new(error: StandardError.new("Previous error"))
        rescued_result = error_result.rescue(&:message)

        expect(rescued_result.value).to be_nil
        expect(rescued_result.error).to eq(StandardError.new("Previous error"))
      end
    end
  end
end
